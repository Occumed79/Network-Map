...
const legacyP2StylePath = path.join(srcRoot, "core-app-p2-fixes.css");
const ownedP2StylePath = path.join(srcRoot, "core-app-p2.css");

if (fs.existsSync(legacyP2StylePath)) {
  if (fs.existsSync(ownedP2StylePath)) throw new Error("core-app-p2.css already exists while legacy P2 fix layer remains");
  fs.renameSync(legacyP2StylePath, ownedP2StylePath);
  let main = fs.readFileSync(mainPath, "utf8");
  if (!main.includes('import "./core-app-p2-fixes.css";')) throw new Error("Legacy P2 stylesheet import missing");
  main = main.replace('import "./core-app-p2-fixes.css";', 'import "./core-app-p2.css";');
...
if (!ownership.includes("Current consolidation progress:")) {
  const consolidationProgress = [
    ...
    "- Renamed the retained P2 feature stylesheet from `core-app-p2-fixes.css` to the feature-owned `core-app-p2.css` without changing its rules.",
...
inventory = inventory.replace(
  ...
  "Stylesheet ownership is now materially consolidated: `ui-system.css` owns shared design tokens/global UI/dialog/popup/report behavior, ten superseded override layers have been deleted, the retained P2 feature rules live in `core-app-p2.css`, and the app stylesheet-import ceiling is 22. Remaining work is to reconcile the smaller set of transitional map/sidebar compatibility files into their owning feature or `ui-system.css` without changing rendered behavior.",
);
fs.writeFileSync(inventoryPath, inventory);

console.log("Tightened stylesheet no-growth guard to 22 imports, retired three legacy theme layers, and renamed the retained P2 stylesheet to feature ownership.");
