...
function first(row: IncomingClinicRow, ...keys: string[]): string {
  ...
  for (const [key, value] of Object.entries(row)) normalized.set(key.toLowerCase().replace(/[\s_-]+/g, ""), value);
...
function classify(row: IncomingClinicRow, name: string, notes: string, taxonomy: string): { primaryProviderType: string; capabilityTags: string[] } {
  ...
  const add = (type: string) => { tags.add(type); if (primary === "unknown") primary = type; };
...
function normalizeRow(row: IncomingClinicRow, index: number): NormalizedClinicRow {
  const name = first(row, "name", "providerName", "provider_name", "clinicName", "clinic_name", "practiceName", "facility", "facilityName") || "Unnamed clinic";
...
function masterKeyFor(row: NormalizedClinicRow): string {
  ...
  return `loc:${contentHash({ name: row.normalizedName, address: row.address.toLowerCase(), city: row.city.toLowerCase(), state: row.state.toLowerCase(), postal: row.postalCode })}`;
}
...
router.post("/my-clinics/upload", async (req: Request, res: Response) => {
  ...
  const rows = Array.isArray(req.body?.rows) ? req.body.rows as IncomingClinicRow[] : Array.isArray(req.body?.clinics) ? req.body.clinics as IncomingClinicRow[] : [];
  if (!rows.length) {
  ...
  try {
    ...
    const batch = await client.query(
      `INSERT INTO public.provider_ingest_batches (source_key, upload_label, upload_method, original_filename, uploaded_by, status, total_rows, metadata)
       VALUES ($1, $2, 'front_end_upload', $3, $4, 'raw_loaded', $5, $6::jsonb)
       RETURNING id`,
      [SOURCE_KEY, groupName, originalFilename, uploadedBy, rows.length, JSON.stringify({ frontend: true })],
    );
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      const rowError = normalized.name === "Unnamed clinic" ? "missing_name" : null;

      const raw = await client.query(
        `INSERT INTO public.provider_raw_records (batch_id, source_key, source_record_id, file_row_number, content_hash, raw_payload, raw_text, status, error_message)
      ...
      if (rowError) { errorRows += 1; continue; }
      ...
      const stage = await client.query(
        `INSERT INTO public.provider_stage_records (
          ...
          primary_provider_type, capability_tags, confidence_score, normalization_status, normalized_payload
        ) VALUES (
      ...
      const master = await client.query(
        `INSERT INTO public.provider_master (
          master_key, name, normalized_name, address_line1, formatted_address, city, state_region,
          postal_code, country_code, lat, lng, phone, website, npi, primary_provider_type,
          capability_tags, primary_source_key, quality_score, last_seen_at, updated_at
        ) VALUES (
          ...
          npi = COALESCE(EXCLUDED.npi, provider_master.npi),
      ...
      await client.query(
        `INSERT INTO public.provider_master_sources (master_provider_id, stage_record_id, raw_record_id, source_key, source_record_id, source_url, source_confidence_score, raw_payload)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb)`,
        [master.rows[0].id, stage.rows[0].id, raw.rows[0].id, SOURCE_KEY, normalized.sourceRecordId, normalized.website || null, normalized.qualityScore, JSON.stringify(normalized.raw)],
      );

      const legacySourceId = `${SOURCE_KEY}:${masterKey}`;
      await client.query(
        `INSERT INTO public.medical_providers (
          place_id, name, formatted_address, lat, lng, types, category, phone, website, country_code,
          locality, administrative_area_level_1, postal_code, data_source, source_id, source_type,
          confidence_score, raw_data, scraped_at, updated_at
        ) VALUES (
    ...
    await client.query(
      `UPDATE public.provider_ingest_batches
       SET status=$2, raw_rows=$3, staged_rows=$4, mastered_rows=$5, error_rows=$6, completed_at=now(), updated_at=now()
       WHERE id=$1`,
      [batchId, errorRows ? "completed_with_errors" : "completed", rawRows, stagedRows, masteredRows, errorRows],
    );
    ...
    res.status(201).json({
      ...
      mirroredRows,
      errorRows,
      message: `Uploaded ${masteredRows} mapped My Clinics rows to provider_master.`,
    });
  } catch (error) {
