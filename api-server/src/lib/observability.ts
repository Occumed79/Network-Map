...
type TimingEvent = { requestId?: string; at: string; kind: string; name: string; durationMs: number; ok: boolean; detail?: Record<string, unknown> };
...
export function recordTiming(kind: string, name: string, durationMs: number, ok: boolean, detail?: Record<string, unknown>) {
  pushBounded(timingEvents, { requestId: currentRequestId(), at: new Date().toISOString(), kind, name, durationMs: Math.max(0, Math.round(durationMs)), ok, detail });
}
