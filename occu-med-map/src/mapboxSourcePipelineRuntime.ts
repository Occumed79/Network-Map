...
export type MapboxSourceDataContext = {
  map: mapboxgl.Map;
  sourceId: string;
  writer: MapboxSourceWriter;
};
...
type RegisteredMiddleware = MapboxSourceDataMiddleware & {
  id: string;
  sourceId: string;
  priority: number;
  sequence: number;
...
type PipelineSource = mapboxgl.GeoJSONSource & {
  __networkMapSourcePipelineWrapped?: boolean;
};
...
declare global {
  interface Window {
    __NETWORK_MAP_MAPBOX_SOURCE_PIPELINE__?: {
...
function emit(phase: string, detail: Record<string, unknown> = {}): void {
  window.dispatchEvent(new CustomEvent("network-map:mapbox-source-pipeline", {
    detail: {
      phase,
...
function orderedMiddleware(sourceId: string): RegisteredMiddleware[] {
  return [...middlewares.values()]
    .filter((middleware) => middleware.sourceId === sourceId)
    .sort((left, right) =>
...
function statsFor(id: string): MiddlewareStats {
  ...
  if (!stats) {
    stats = { transformedWrites: 0, suppressedWrites: 0 };
    middlewareStats.set(id, stats);
  }
  return stats;
}

function applyMiddleware(
  context: MapboxSourceDataContext,
  data: MapboxGeoJsonData,
): { allowed: boolean; data: MapboxGeoJsonData; suppressedBy?: string } {
  let nextData = data;
...
function sourceState(map: mapboxgl.Map, sourceId: string): SourceState | undefined {
  return statesByMap.get(map)?.get(sourceId);
}
...
function storeState(state: SourceState): void {
  ...
  if (!states) {
    states = new Map();
    statesByMap.set(state.map, states);
  }
...
function applySourceData(
  ...
  const context: MapboxSourceDataContext = {
    map: state.map,
  ...
  if (!result.allowed) {
    ...
    emit("write-suppressed", {
      sourceId: state.sourceId,
      writer,
      middlewareId: result.suppressedBy,
    });
    return state.source;
  }
...
function wrapSource(map: mapboxgl.Map, sourceId: string): SourceState | null {
  ...
  const state: SourceState = {
    ...
    sourceId,
    source,
    nativeSetData: source.setData.bind(source),
    writesApplied: 0,
    writesSuppressed: 0,
  };
...
function transformInitialSource(
  map: mapboxgl.Map,
  sourceId: string,
  ...
  return result.data === specification.data
    ? source
    : { ...specification, data: result.data } as mapboxgl.GeoJSONSourceSpecification;
}
...
function installPipelineOwner(): void {
  ...
  const nativeAddSource = prototype.addSource as (
    this: mapboxgl.Map,
    id: string,
    source: mapboxgl.AnySourceData,
  ) => mapboxgl.Map;
  const nativeRemoveSource = prototype.removeSource as (
    this: mapboxgl.Map,
    id: string,
  ) => mapboxgl.Map;

  prototype.addSource = function pipelineAddSource(
    this: mapboxgl.Map,
    id: string,
    source: mapboxgl.AnySourceData,
  ): mapboxgl.Map {
...
export function registerMapboxSourceDataMiddleware(
  middleware: MapboxSourceDataMiddleware,
): () => void {
  ...
  const registered: RegisteredMiddleware = {
    ...
    id,
    sourceId,
    priority: Number.isFinite(middleware.priority) ? Number(middleware.priority) : 100,
    sequence: sequence += 1,
  };
  ...
  return () => {
    if (middlewares.get(id) !== registered) return;
    middlewares.delete(id);
    emit("middleware-unregistered", { id, sourceId });
  };
}

export function setMapboxGeoJsonSourceData(
  map: mapboxgl.Map,
  sourceId: string,
  data: MapboxGeoJsonData,
  writer: MapboxSourceWriter,
): mapboxgl.GeoJSONSource | null {
  ...
  return {
    ...
    middlewares: [...middlewares.values()]
      ...
      .map((middleware) => {
        ...
        return {
          id: middleware.id,
          sourceId: middleware.sourceId,
          priority: middleware.priority,
          transformedWrites: stats.transformedWrites,
          suppressedWrites: stats.suppressedWrites,
        };
      }),
    sources,
  };
...
window.__NETWORK_MAP_MAPBOX_SOURCE_PIPELINE__ = {
  getDiagnostics: getMapboxSourcePipelineDiagnostics,
};
