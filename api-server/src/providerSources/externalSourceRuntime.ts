...
function linkedSignal(parent: AbortSignal | undefined, timeoutMs: number, sourceId: string) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(new ExternalSourceError(`Source timeout after ${timeoutMs}ms`, sourceId, "timeout")), timeoutMs);
...
export async function fetchExternalJson<T>(
  ...
  try {
    for (let attempt = 1; attempt <= state.policy.maxAttempts; attempt += 1) {
      ...
      const linked = linkedSignal(options.signal, state.policy.timeoutMs, sourceId);
      try {
