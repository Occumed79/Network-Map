...
try {
  ...
  globalThis.fetch = (async () => {
    cacheFetches += 1;
    return new Response(JSON.stringify({ cached: true }), { status: 200, headers: { "content-type": "application/json" } });
  }) as typeof fetch;
  await fetchExternalJson("overpass", "https://example.test/cache", {}, { cache: true });
  await fetchExternalJson("overpass", "https://example.test/cache", {}, { cache: true });
  assert.equal(cacheFetches, 1, "source cache must avoid repeated identical external calls inside TTL");

  clearExternalSourceRuntimeForTests();
  const controller = new AbortController();
  globalThis.fetch = (async (_input: any, init?: RequestInit) => new Promise<Response>((_resolve, reject) => {
    init?.signal?.addEventListener("abort", () => reject(new DOMException("Cancelled", "AbortError")), { once: true });
  })) as typeof fetch;
  const cancelled = fetchExternalJson("npi", "https://example.test/cancel", {}, { signal: controller.signal, cache: false });
  controller.abort();
  await assert.rejects(cancelled, (error: unknown) => error instanceof ExternalSourceError && error.code === "cancelled");
  ...
  assert.doesNotMatch(osm, /await\s+fetch\(/, "Overpass adapter must not bypass the resilience runtime");
  assert.match(geocode, /fetchExternalJson/, "geocoders must use shared external-source runtime");
  assert.doesNotMatch(geocode, /await\s+fetch\(/, "geocoders must not bypass the resilience runtime");
  assert.match(geocode, /"geocodio"[\s\S]*"nominatim"/, "both geocoders must have source-specific resilience policies");
  assert.match(orchestrator, /degradedSources/, "partial searches must explicitly report degraded sources");
  assert.match(orchestrator, /incomplete:\s*degradedSources\.length > 0/, "partial results must be explicitly marked incomplete");
  assert.match(sourceStatus, /\/source-health/, "source health must be separate from application liveness/readiness");
  assert.match(sourceStatus, /getExternalSourceHealth/, "source-health endpoint must expose shared runtime health");
} finally {
  globalThis.fetch = originalFetch;
  clearExternalSourceRuntimeForTests();
}

console.log("External source timeout/retry/circuit/concurrency/cache/cancellation/degraded-state hardening smoke passed.");
