...
SELECT (
  ...
  AND BOOL_AND(source_record_id LIKE 'cnes:%')
...
CREATE TEMP TABLE cnes_old_master_ids ON COMMIT DROP AS
SELECT DISTINCT master_provider_id
FROM public.provider_master_sources
WHERE source_key = 'br_cnes';
...
VALUES
  ('br_cnes', 'Brazil CNES — Cadastro Nacional de Estabelecimentos de Saúde', 'government_registry', 'registry', true,
   'Official Brazilian Ministry of Health national establishment registry; synchronized from the OpenDataSUS CNES bulk JSON export')
...
SELECT
  r.id, 'br_cnes', t.source_record_id, t.name,
...
SELECT
  ...
  capability_tags, 'br_cnes', quality_score, true, now(), now()
FROM (
...
ON CONFLICT (master_key) DO UPDATE SET
  ...
  primary_source_key = CASE
    ...
    THEN 'br_cnes' ELSE provider_master.primary_source_key END,
...
SELECT
  pm.id, s.id, r.id, 'br_cnes', t.source_record_id,
...
SELECT DISTINCT
  pm.id, type_key, 'br_cnes', t.quality_score
FROM public.source5_import_staging t
JOIN public.provider_master pm ON pm.master_key = t.master_key
