...
const projectCount = Number(argument("project-count", "4"));
if (!Number.isInteger(projectCount) || projectCount < 1 || projectCount > 4) {
  throw new Error("--project-count must be an integer from 1 through 4");
}
...
if (!sourceRows || !header) throw new Error("No normalized USA Embassy records were found");
...
const streams = projects.map((project) => {
  const stream = fs.createWriteStream(path.join(outputDirectory, `usa_embassy_provider_project_${project.slot}.tsv`));
  stream.write(`${header}\n`);
  return stream;
});
...
await forEachDataLine(async ({ line, sourceRecordId, countryCode, masterKey }) => {
  ...
  if (!slot) throw new Error(`No Embassy provider project allocation exists for ${countryCode}`);
...
if (projects.reduce((sum, project) => sum + project.rows, 0) !== sourceRows) {
  throw new Error("Embassy provider project row totals do not match source rows");
}

const manifest = {
  generated_at: new Date().toISOString(),
  source: "Final geocoded U.S. Embassy medical provider workbooks",
  source_key: "embassy_clinic_docs",
  source_rows: sourceRows,
  project_count: projectCount,
  strategy: "Whole countries are greedily balanced by provider count; records without a country code are deterministically distributed.",
  projects: projects.map((project) => ({
    slot: project.slot,
    environment_variable: project.slot === 1 ? "USA_EMBASSY_DATABASE_URL" : `USA_EMBASSY_DATABASE_URL_${project.slot}`,
    ...
    file: `usa_embassy_provider_project_${project.slot}.tsv`,
  })),
};
