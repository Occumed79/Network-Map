...
assert.match(css, /min-height: 100dvh/, "the recovery screen must fill the viewport");
assert.match(css, /max-height: calc\(100dvh - 48px\)/, "recovery content must remain viewport constrained");
assert.match(css, /@media \(forced-colors: active\)/, "recovery controls must support forced-colors mode");
assert.match(css, /env\(safe-area-inset-top\)/, "recovery layout must respect mobile safe areas");
