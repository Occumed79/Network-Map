...
export default function DatasetBrowser({ open,onClose,getMapBounds,getCurrentRadius,onViewOnMap,onViewDensity,onCompare,sharedFilters,onFiltersChange,onOpenMatchingInDatabase }:Props) {
  ...
  useEffect(()=>{ if(!open) return; const ac = new AbortController(); (async()=>{ setLoading(true); setError(''); setWarning(''); try { const [recordsResp, facetsResp] = await Promise.all([ fetch(`/api/provider-explorer?${buildParams('records')}`, {signal:ac.signal}), fetch(`/api/provider-explorer/facets?${buildParams('facets')}`, {signal:ac.signal}) ]); const data = await recordsResp.json() as ProviderExplorerResponse; const facetData = await facetsResp.json().catch(()=>({facets:[]})); if(!recordsResp.ok || data.error) throw new Error(data.error || `HTTP ${recordsResp.status}`); setRows(data.providers || data.records || []); setTotal(Number(data.total || 0)); setHasMore(Boolean(data.hasMore)); if(data.status) setExplorerStatus(data.status); setFacets(Array.isArray((facetData as any).facets) ? (facetData as any).facets : []); setWarning(data.warning || ''); } catch(e) { if(!ac.signal.aborted) setError(e instanceof Error ? e.message : 'Provider explorer request failed'); } finally { if(!ac.signal.aborted) setLoading(false); } })(); return ()=>ac.abort(); },[open,buildParams]);

  useEffect(()=>{ if(!open) return; const ac = new AbortController(); (async()=>{ try { const resp = await fetch('/api/provider-explorer/status', {signal:ac.signal}); const data = await resp.json() as ProviderExplorerStatus; if(resp.ok && !ac.signal.aborted) setExplorerStatus(data); } catch { /* status is non-blocking */ } })(); return ()=>ac.abort(); },[open]);

  async function postAction(path:string, provider:ProviderFeature, label:string) { setActionStatus(`${label}…`); try { const resp = await fetch(path, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({provider, id:provider.id}) }); const data = await resp.json().catch(()=>({})); if(!resp.ok || data.error) throw new Error(data.error || data.message || `HTTP ${resp.status}`); setActionStatus(data.message ? `${label}: ${data.message}` : `${label} complete`); } catch(e) { setActionStatus(`${label} failed: ${e instanceof Error ? e.message : 'Provider Explorer action failed'}`); } }
  ...
  return <div className="dataset-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget) onClose();}}><section className="dataset-modal" role="dialog" aria-modal="true" aria-label="Provider Database Explorer">
    ...
    {explorerStatus && <div className="provider-explorer-runtime-status">Persistence: {explorerStatus.persistenceConfigured ? 'yes' : 'no'} · Spatial: {explorerStatus.spatialEngine || 'unknown'} · Candidates: {explorerStatus.candidatePersistence ? 'yes' : 'no'} · Saved: {explorerStatus.savedPersistence ? 'yes' : 'no'} · Live: {(explorerStatus.liveAdapters || []).join(', ') || 'none'} · Schema: {explorerStatus.schema || 'unknown'}</div>}
